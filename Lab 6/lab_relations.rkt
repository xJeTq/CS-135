...
(define (compose-helper x L)
  (if (null? L) '() (append (list (append (list x) (list (car L)))) (compose-helper x (cdr L)))))

(define (compose S R)
  (if (null? R) '() (if (null? S) '() (if (not (null? (relate (cadar R) S))) (make-set (append (compose-helper (caar R) (relate (cadar R) S)) (compose S (cdr R)))) (compose S (cdr R))))))  
...
(define (power-helper R P k)
  (if (equal? k 1) P (power-helper R (compose R P) (- k 1))))

(define (power k R)
      (if (equal? k 0) '() (power-helper R R k)) 
)
...
(define (transitive-closure R)
   (define (transitive-closure-helper num R) (if (equal? num 0) '() (if (equal? num 1) R (union (power num R) (transitive-closure-helper (- num 1) R)))))(let loop ((count 0))(if (set-equal? (transitive-closure-helper count R) (transitive-closure-helper (+ 1 count) R))(transitive-closure-helper count R)(loop (+ count 1)))))
...
(define (surjective? n R)
  (define (surjectiveH? R)(if (null? R)'()(append (list (car (cdr (car R)))) (surjectiveH? (cdr R)))))(if (equal? n 0) #t (if (element? (surjectiveH? R) n) (surjective? (- n 1) R) #f)))

(define (involutive? n R)
   (if (surjective? n R) (if (injective? R) (if (symmetric? R) #t #f) #f) #f))
...
;; Type signature: (function? relation) -> boolean
;; 3 Extra Credit PTS
(define (function? R)
  "implement"
)
