...
(define (cong-mod? m)
(lambda (x y) (if (equal? (remainder x m) (remainder y m)) #t #f))
)
...
(define (pulverizer a b)
  (if (= a 0) (list 0 1) (let* ((res (pulverizer (modulo b a) a)) (x1 (car res)) (y1 (cadr res)) (x (- y1 (* (quotient b a) x1))) (y x1)) (list x y)))
)
...
(define (mod-inverse a m)
  (let ((co (pulverizer a m))) (if (= (gcd a m) 1) (if (< (car co) 0) (+ (car co) m) (car co)) (error-inverse)))
)