...
;; 2 PTS
(define (p- a b)
  (if (pz? b) a (if (pz? a) (error-subtraction) (p- (pred a) (pred b))))
)
...
;; 2 PTS
(define (p* a b)
  (if (or (pz? a) (pz? b)) z (p+ a (p* a (pred b))))
)
...
;; 1 PTS
(define (p^ a b)
  (if (pz? a) a (if (pz? b) (succ z) (p* a (p^ a (pred b)))))
)
...
;; 2 PTS
(define (p= a b)
  (if (and (pz? a) (pz? b)) #t (if (or (pz? a) (pz? b)) #f (p= (pred a) (pred b))))
)
...
(define (p> a b)
  (if (and (pz? a) (pz? b)) #f (if (pz? b) #t (if (pz? a) #f (if (p= a b) #f (p> (pred a) (pred b))))))
)
...
;; 2 PTS
(define (p% a b)
  (if (pz? b) (error-modulo) (if (not (p> a b)) a (p% (p- a b) b)))
)
...
(define (p-divides? a b)
  (if (and (not (pz? a)) (pz? (p% b a))) #t (if (and (pz? a) (pz? b)) #t #f))
)
...
(define (prime-helper? p a)
  (if (p= p a) #t (if (p-divides? a p) #f (prime-helper? p (succ a))))
)
