...
#| In this lab, we'll implement some basic functions which operate on multiple sets.
 ...
 | If the output is expecting a set, make sure your tail
...
(define (intersection S1 S2)
  (make-set (if (null? S1) '() (if (element? S2 (car S1)) (cons (car S1) (intersection (cdr S1) S2)) (intersection (cdr S1) S2))))
)
...
(define (set-equal? S1 S2)
  (if (null? S1) #t (if (element? S2 (car S1)) (set-equal? (cdr S1) S2) #f))
)
...
(define (difference S1 S2)
  (make-set (if (null? S1) '() (if (element? S2 (car S1)) (difference (cdr S1) S2) (cons (car S1) (difference (cdr S1) S2)))))
)
