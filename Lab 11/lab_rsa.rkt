...
(define (generate p q e)
  (let ((n (* p q)) (phi (lcm (- p 1) (- q 1))) (d (mod-inverse e (lcm (- p 1) (- q 1))))) (list (list n e) (list n d)))
)
...
(define (mod-exp b e m)
  (cond ((= e 0) 1) ((even? e) (modulo (expt (mod-exp b (/ e 2) m) 2) m)) (else (modulo (* b (mod-exp b (- e 1) m)) m)))
)
...
(define (encrypt-Helper c key)
  (if (null? c) '() (cons (mod-exp (char->integer (car c)) (cadr key) (car key)) (encrypt-Helper (cdr c) key)))
)
...
(define (decrypt-Helper ciphertext key)
  (if (null? ciphertext) '() (cons (integer->char (mod-exp (car ciphertext) (cadr key) (car key))) (decrypt-Helper (cdr ciphertext) key)))
)
