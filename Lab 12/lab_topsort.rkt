...
(define (indegree E v)
 (if (null? E) 0 (if (equal? (cadar E) v) (+ 1 (indegree (cdr E) v)) (indegree (cdr E) v)))
)
...
(define (find-topHelper A B)
 (if (null? A) (topsort-error) (if (= 0 (indegree B (car A))) (car A) (find-topHelper (cdr A) B)))
)
...
(define (pop-vertex G v)
 (define L1 (popv-vertices (get-vertices G) v)) (define L2 (popv-edges (get-edges G) v)) (cons L1 (list L2))
)
...
(define (topsort G)
 (if (null? (get-vertices G)) '() (cons (find-top G) (topsort (pop-vertex G (find-top G)))))
  )
